...
@interface S4M_069_EvalLetStarTestCase : XCTestCase

@end

@implementation S4M_069_EvalLetStarTestCase
{
...
- (void)testLetStarWithMultipleExpressions {
    [inputStream setStream:@"(let* ((x 10) (y (+ x 10)) (z (+ y 10))) (define x 50) (+ x y z))"];
    ...
    XCTAssertTrue([[outputStream getStream] isEqualToString:@"100"], @"let star doesn't work with multiple expressions: %@", [outputStream getStream]);
}

- (void)testLetStarWithEmptyExpressionBlock {
    [inputStream setStream:@"(let* ((x 10) (y (+ x 10)) (z (+ y 10))) ())"];
    parsedResult = [[S4MReader sharedInstance] readSchemeObjectFromStream:inputStream];
    XCTAssertThrowsSpecificNamed([[S4MEval sharedInstance] evalObject:parsedResult inEnvironment:nil]
                                 , NSException, @"Missing Procedure Exception", @"doesnt throw right exception!");
}
